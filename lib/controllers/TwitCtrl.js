...
module.exports = {
    ...
    retweet: function(req, res) {
        T.post('statuses/retweet/' + req.params.tweet_id , {id: req.params.tweet_id}, function(err, data, response){
            if(!err)
                res.status(200).json(data);
            else
                console.log(err);
    ...
    favorite: function(req, res) {
        ...
        T.post('favorites/create', {id: req.params.tweet_id}, function(err, data, response){
            if(!err)
                res.status(200).json(data);
            else
                console.log(err);
